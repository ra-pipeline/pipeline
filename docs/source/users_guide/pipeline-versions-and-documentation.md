...
A link to the CASA+pipeline package is available, along with installation instructions and supporting documentation, from the **Overview and Pipeline** section of the [ALMA Science Portal](http://www.almascience.org) (under the "Processing" tab, or directly at [almascience.org/processing](http://almascience.org/processing/)). If any issues are encountered with CASA installation, please contact the [ALMA Helpdesk](https://help.almascience.org/) via the link on the ALMA Science Portal.
...
Examples of common re-imaging modifications to the IF pipeline script are given in the [ALMA Imaging Pipeline Reprocessing Casaguide](https://casaguides.nrao.edu/index.php/ALMA_Imaging_Pipeline_Reprocessing).

An example of the pipeline processing for the total power data is available in the [Single-Dish Data Processing Casaguide](https://casaguides.nrao.edu/index.php/Single-Dish_(Total_Power)_Data_Processing_with_Pipeline).
...
CASA produces numerous releases, but only the versions listed in the table on the [ALMA Science Portal](http://almascience.org/processing/) have been scientifically validated, are accepted for operations, and are supported by ALMA. That table additionally lists the versions which can be used to restore previously pipeline-calibrated archival visibility data.

Note that each CASA release often includes one or more updates to the third-party Python modules that are packaged with it. A summary of the changes in the CASA versions used between PL2023, PL2024, PL2025, and PL2026 are shown in the table below.
...
<sup>a</sup> contained the CASA task `wvrgcal` (legacy `almatasks` package), which was migrated to {func}`casatasks.wvrgcal <casatasks.calibration.wvrgcal>` in PL2025.

## Pipeline and CASA tasks

(sec-pipelinecasa)=
The pipeline heuristics are written as python functions appearing with a `hif_` or `hifa_` (for interferometric) or `hsd_` (for single-dish) prefix. They can be viewed and executed within CASA just as any python function (if one has launched CASA with "--pipeline"). For example, one can view the possible inputs for the task {func}`~pipeline.hifa.cli.hifa_importdata` by typing `?hifa_importdata`.

The pipeline heuristics use CASA tasks wherever possible to perform the data reduction or imaging. E.g. the pipeline bandpass calibration & flagging task {func}`~pipeline.hifa.cli.hifa_bandpassflag` calls the CASA {func}`~casatasks.calibration.bandpass` task, and the interferometric imaging task {func}`~pipeline.hif.cli.hif_makeimages` calls the CASA imaging task {func}`~casatasks.imaging.tclean`.

The standard pipeline processing recipes are deterministic and should always give the same result for the same data, if run on the same hardware allocation (number of cores, memory). However, the Pipeline tasks are designed to be highly flexible, so that they can have the default inputs over-ridden with user-specified values, or be added, subtracted, or rearranged to produce alternative processing recipes. This enables a manual "mix and match" mode for data reduction and imaging that combines standard Pipeline tasks with other CASA commands or python code to produce scripts that are better tuned to the idiosyncrasies of a specific dataset. The exact pipeline commands that will reproduce the standard recipe are delivered with each dataset, in a script called `member.<mous_uid>.<recipe>.casa_pipescript.py` (see the {ref}`Pipeline processing script <sec-pipescriptintro>`). One can edit and add to that script to implement "mixed mode" processing.

Some common "manual mode" modifications are presented in {ref}`Pipeline re-processing considerations <sec-pipescriptreprocess>`. A complete list of the variables for each pipeline task is given in the API sections of this site.
